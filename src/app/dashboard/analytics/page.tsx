...
function MetricCard({ label, value, trend, icon, delay }: MetricCardProps) {
  ...
  return (
    <div className="card-metric" style={{ animation: `fadeIn 0.5s ease ${delay}ms both` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        <div
          style={{
            ...
            fontSize: 12, fontWeight: 500, color: trendColor,
          }}
        >
          <TrendIcon size={14} />
          {Math.abs(trend)}%
      ...
      <div style={{ fontSize: 24, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 4, letterSpacing: '-0.04em' }}>{value}</div>
  ...
  return (
    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
      <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Click Heatmap</h3>
...
function InsightsPanel() {
  ...
  return (
    <div className="card" style={{ marginBottom: 28 }}>
      <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
        {insights.map((insight) => {
          ...
          return (
            ...
            >
              <div style={{ color: 'var(--brand-cyan)', flexShrink: 0, marginTop: 2 }}>
                {React.cloneElement(severityIcons[insight.severity] as React.ReactElement<any>, { size: 18 })}
              </div>
              <div style={{ flex: 1 }}>
...
function NetworkHealthPanel() {
  const stats = [
    { label: 'Server Uptime', value: '99.98%', icon: <Server size={18} />, status: 'success' },
    { label: 'Cache Hit Rate', value: '87.4%', icon: <Database size={18} />, status: 'success' },
    { label: 'CPU Usage', value: '14.2%', icon: <Cpu size={18} />, status: 'info' },
  ];

  return (
    <div className="card" style={{ height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {stats.map((stat, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ color: 'var(--brand-cyan)' }}>{React.cloneElement(stat.icon as React.ReactElement<any>, { size: 18 })}</div>
              <div style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{stat.label}</div>
            </div>
            <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{stat.value}</div>
          </div>
        ))}
        
        <div style={{ marginTop: 8, padding: 12, background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', marginBottom: 8 }}>Recent Incidents</div>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 4 }}>Analytics Overview</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: canSeeSystemData ? '2fr 1.2fr 1fr' : '2fr 1fr', gap: 20, marginBottom: 24 }}>
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Traffic & Engagement</h3>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Device Distribution</h3>
          </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginTop: 20, justifyContent: 'center' }}>
              {deviceData.map((d) => (
                <div key={d.device} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-primary)' }}>{d.value}%</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 20, marginBottom: 28 }}>
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Acquisition Channels</h3>
          </div>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Conversion Funnel</h3>
          </div>
      ...
      <div style={{ marginTop: 40, borderTop: '2px solid var(--border-color)', paddingTop: 40, marginBottom: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <h2 style={{ fontSize: 20, fontWeight: 500, color: 'var(--text-primary)' }}>Email Marketing Subscriber Analytics</h2>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
          ...
          <div className="card" style={{ padding: 0 }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h3 style={{ fontSize: 15, fontWeight: 500, color: 'var(--text-primary)' }}>Subscriber Growth</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="card" style={{ flex: 1, padding: 0 }}>
              <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-color)' }}>
                <h3 style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-primary)' }}>New vs Returning</h3>
              </div>
                  ...
                  {subscriberTypeData.map((t) => (
                    <div key={t.name}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-primary)' }}>{t.value}</span>
                      </div>
            ...
            <div className="card" style={{ flex: 1.5, padding: 0 }}>
              <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border-color)' }}>
                <h3 style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-primary)' }}>Subscriber Sources</h3>
              </div>
                    ...
                    return (
                      <div key={s.source}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                          ...
                          <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-primary)' }}>{pct}%</span>
                        </div>
